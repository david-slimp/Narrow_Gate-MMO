// Narrow Gate MMO - Christian-themed rogue-like MMO dungeon crawler
// Copyright (C) 2025-2026 David Slimp (rock808@David-Slimp.com)
// Licensed under the GNU AGPLv3 or later. See LICENSE.txt.
...
// Sprite mapping for tile types
// the numbers we are using here start at "1" rather than "0" cuz us humans prefer it that way
const spriteMapping = {
  '#': { x: 1, y: 1 },  // Permanant Wall
  '*': { x: 6, y: 1 },  // Placed Wall / Obstacle
  '~': { x: 7, y: 2 },  // Lava
  '.': { x: 2, y: 1 },  // Floor
  '$': { x: 1, y: 3 },  // Faith
  'H': { x: 4, y: 2 },  // Health
  ...
  '@': { x: 4, y: 5 },  // PlayerM
  '%': { x: 5, y: 5 },  // PlayerF
  'D': { x: 2, y: 2 },  // Door Forward
  'd': { x: 1, y: 2 },  // Door Backward
  't': { x: 1, y: 4 },  // Temptation
  's': { x: 7, y: 4 }   // Sorrow
};
...
// Define available player sprites (columns on row 5)
const availableSprites = [
  { col: 4, row: 5 },
  { col: 5, row: 5 },
  // Add more here later { col: X, row: 5 }
];

//const PLAYER_SPRITE_POS = { col: 4, row: 5 }; // We use availableSprites[0] now
...
function loadArea(areaId) {
  ...
  if (fs.existsSync(file)) {
    ...
    grid = generateGrid(areaId); // Pass areaId
    placeDoors(grid);
...
function spawnMonstersInArea(areaId) {
  const area = areas[areaId]; if (!area) return;
  const isArea00001 = areaId === '00001'; // Check if it's the target area
  const empties = [];
  area.grid.forEach((row,y)=> row.forEach((c,x)=>{
    // Standard empty check + Area 00001 boundary check
    if (c==='.' &&
        (!isArea00001 || (x <= 40 && y <= 40)) && // <-- ADDED CHECK
        !playersInArea(areaId).some(p=>p.x===x&&p.y===y) &&
        !area.monsters.some(m=>m.x===x&&m.y===y))
    {
        empties.push({x,y});
    }
  }));
...
function generateGrid(areaId) { // Added areaId parameter
    ...
    let faithPlaced = 0;
    let faithAttempts = 0;
    while (faithPlaced < 5 && faithAttempts < 1500) { // Limit attempts
        faithAttempts++;
        let x, y;
        let findSpotAttempts = 0;
        // 1. Find a random empty spot
        do {
            x = Math.floor(Math.random() * gridSize);
            y = Math.floor(Math.random() * gridSize);
            findSpotAttempts++;
            if (findSpotAttempts > 1000) {
                console.warn(`[generateGrid] Could not find an empty ('.') spot for Faith in area ${areaId} after 1000 checks.`);
                break; // Break inner loop
            }
        } while (grid[y]?.[x] !== '.');

        if (findSpotAttempts > 1000) break; // Break outer loop

        // 2. Check Area 00001 restriction
        if (areaId === '00001' && x > 35 && y > 35) {
            if (DEBUG >= 3) console.log(`[generateGrid] Skipping Faith placement at (${x},${y}) in Area 00001 due to >35 rule.`);
            continue; // Skip this spot
        }

        // 3. Place Faith if checks pass
        grid[y][x] = '$';
        faithPlaced++;
    }
    if (faithPlaced < 5) {
        console.warn(`[generateGrid] Placed only ${faithPlaced}/5 Faith tiles in area ${areaId} after ${faithAttempts} attempts.`);
    }
    // Place Lava ('~')
    let lavaPlaced = 0;
    let lavaAttempts = 0;
    while (lavaPlaced < 9 && lavaAttempts < 2000) { // Limit attempts to prevent infinite loop
        lavaAttempts++;
        let x, y;
        let findSpotAttempts = 0;
        // 1. Find a random empty spot
        do {
            ...
            findSpotAttempts++;
            if (findSpotAttempts > 1000) {
                console.warn(`[generateGrid] Could not find an empty ('.') spot for Lava in area ${areaId} after 1000 checks.`);
                break; // Break inner loop if can't find empty spot
            }
        } while (grid[y]?.[x] !== '.');

        if (findSpotAttempts > 1000) break; // Break outer loop if inner failed

        // 2. Check Area 00001 restriction
        if (areaId === '00001' && x > 45 && y > 45) {
            if (DEBUG >= 3) console.log(`[generateGrid] Skipping Lava placement at (${x},${y}) in Area 00001 due to >45 rule.`);
            continue; // Skip this spot, try finding another one
        }

        // 3. Place Lava if checks pass
        grid[y][x] = '~';
        lavaPlaced++;
    }
    if (lavaPlaced < 9) {
        console.warn(`[generateGrid] Placed only ${lavaPlaced}/9 Lava tiles in area ${areaId} after ${lavaAttempts} attempts.`);
    }
    return grid;
}
...
// Helper to create a player state object safe for sending to clients (removes ws)
function getPlayerStateForClient(player) {
  if (!player) return null;
  const { ws, ...clientSafePlayer } = player; // Destructure to exclude ws
...
function getPlayersInView(player, areaId) {
  ...
  for (const id in players) {
    ...
    if (p.area === areaId && Math.abs(p.x - player.x) <= VIEW_RADIUS && Math.abs(p.y - player.y) <= VIEW_RADIUS) {
      visible[id] = getPlayerStateForClient(p); // Use helper here
    }
  }
  return visible;
}
...
wss.on('connection', (ws, req) => {
    ...
    let playerData = null;
    let saveLoadedSuccessfully = false; // Flag to track if save loaded
    if (fs.existsSync(saveFile)) {
      try {
        playerData = JSON.parse(fs.readFileSync(saveFile, 'utf8'));
        if (DEBUG >= 2) console.log(`Loaded player data from ${saveFile}`);
        if (DEBUG >= 1) console.log('[Debug] Raw loaded playerData:', JSON.stringify(playerData)); // ADDED DEBUG
        saveLoadedSuccessfully = true; // Mark save as loaded
        // Initialize or load ipLoginCounts
        playerData.ipLoginCounts = playerData.ipLoginCounts || {};
        // Increment count for current IP
        playerData.ipLoginCounts[safeIp] = (playerData.ipLoginCounts[safeIp] || 0) + 1;
        if (DEBUG >= 1) console.log(`[IP Counts] Incremented count for ${ip} (${safeIp}) to ${playerData.ipLoginCounts[safeIp]}.`);
      } catch (err) {
        console.error('Failed to load player save:', err);
        playerData = null; // Ensure playerData is null on error
      }
    } else {
        // If no save file exists, initialize ipLoginCounts for the first save
        playerData = { ipLoginCounts: { [safeIp]: 1 } };
        if (DEBUG >= 1) console.log(`[IP Counts] No save file for ${ip}. Initializing count for ${safeIp} to 1.`);
    }
    const isNewPlayer = !saveLoadedSuccessfully;
    const name = isNewPlayer ? "NoName" : (playerData?.name || "NoName"); // Default name to "NoName"
    let startX, startY, playerArea = DEFAULT_AREA; // Default area
    let playerColor;

    // ... (player color selection logic remains the same) ...

    // Determine spawn location
    if (isNewPlayer) {
        // Force new player spawn location
        playerArea = '00001';
        startX = 57;
        startY = 57;
        if (DEBUG >= 1) console.log(`[Spawn] New player (${ip}). Forcing spawn at ${playerArea}:${startX},${startY}.`);
        // Ensure playerData object exists for ipLoginCounts later if needed
        if (!playerData) playerData = {};
    } else {
        // Returning player logic - load area and position
        playerArea = playerData.area || DEFAULT_AREA; // Load saved area or use default
        // Ensure the area exists, fallback to default if not
        if (!areas[playerArea]) {
            console.warn(`[Server] Saved area '${playerArea}' not found for player ${ip}, defaulting to ${DEFAULT_AREA}`);
            playerArea = DEFAULT_AREA;
        }

        // Try loading saved position
        if (playerData.position) {
            const {x, y} = playerData.position;
            // Check if position is valid within the grid boundaries of the loaded area
            // Note: We don't necessarily need it to be '.' for returning players, just within bounds.
            if ( x >= 0 && x < gridSize && y >= 0 && y < gridSize && areas[playerArea]?.grid[y]?.[x] !== undefined ) {
                startX = x;
                startY = y;
                if (DEBUG >= 2) console.log(`[Spawn] Returning player (${ip}). Using saved position (${x},${y}) in area ${playerArea}`);
            } else {
                // Saved position invalid or outside grid bounds - random spawn in determined area
                if (DEBUG >= 2) console.log(`[Spawn] Returning player (${ip}). Saved position (${x},${y}) invalid for area ${playerArea}. Random spawn.`);
                do {
                    startX = Math.floor(Math.random() * gridSize);
                    startY = Math.floor(Math.random() * gridSize);
                } while (areas[playerArea]?.grid[startY]?.[startX] !== '.');
            }
        } else {
             // No position saved - random spawn in determined area
             if (DEBUG >= 2) console.log(`[Spawn] Returning player (${ip}). No position saved. Random spawn in area ${playerArea}.`);
             do {
                startX = Math.floor(Math.random() * gridSize);
                startY = Math.floor(Math.random() * gridSize);
             } while (areas[playerArea]?.grid[startY]?.[startX] !== '.');
    ...
    // Assign initial IP counts correctly based on whether player is new
    const initialIpCounts = isNewPlayer ? { [safeIp]: 1 } : (playerData?.ipLoginCounts || { [safeIp]: 1 });

    if (DEBUG >= 2) console.log(`Assigning player ${playerId} to (${startX}, ${startY}) in area ${playerArea} as ${playerColor} from IP ${ip}`);
    players[playerId] = {
      id: playerId,
      ip: ip, // Store the IP address
      x: startX, y: startY, area: playerArea, // Use determined values
      color: playerColor,
      name: name,
      spriteCoords: isNewPlayer ? availableSprites[Math.floor(Math.random() * availableSprites.length)] : (playerData?.spriteCoords || availableSprites[0]), // Random sprite for new, else loaded/default
      // Load stats using defaults ONLY if it's a new player
      faith: isNewPlayer ? 0 : (playerData?.faith || 0),
      health: isNewPlayer ? 10 : (playerData?.health || 10),
      buster: isNewPlayer ? 0 : (playerData?.buster || 0),
      wall: isNewPlayer ? 0 : (playerData?.wall || 0), // Start new players with 0 walls
      filler: isNewPlayer ? 0 : (playerData?.filler || 0),
      ipLoginCounts: initialIpCounts, // Store loaded/initialized counts
      ws: ws
    };
    ...
    // Send the sprite mapping to the client
    ws.send(JSON.stringify({ type: 'spriteMappingUpdate', mapping: spriteMapping }));
    ...
    ws.on('message', (raw, isBinary) => {
      ...
      switch (type) {
        ...
        case 'drop':
          ...
          // Send full update including potentially changed player stats
          broadcastState(players[playerId].area);
          break;
        case 'changeSprite':
          if (!players[playerId]) return;
          const currentSprite = players[playerId].spriteCoords;
          let currentIndex = availableSprites.findIndex(s => s.col === currentSprite.col && s.row === currentSprite.row);
          if (currentIndex === -1) currentIndex = 0; // Default to first if not found
          const nextIndex = (currentIndex + 1) % availableSprites.length;
          players[playerId].spriteCoords = availableSprites[nextIndex];
          if (DEBUG >= 2) console.log(`[Server] Player ${playerId} changed sprite to col:${availableSprites[nextIndex].col}, row:${availableSprites[nextIndex].row}`);
          // Send update to show new sprite
          broadcastState(players[playerId].area);
          break;
        case 'chat':
...
function sendView(ws, type) {
  ...
  ws.send(JSON.stringify({
    ...
    players: getPlayersInView(player, area), // getPlayersInView already uses the helper
    monsters: getMonstersInView(player, areas[area].monsters)
  }));
}

// Broadcast viewport updates for 'players' or 'update' types
...
// Broadcast general data, using viewport broadcasts for specific types
function broadcast(data) {
  if (DEBUG >= 7) console.log(`[Server] broadcast: type=${data.type}`);

  // Special handling for statsUpdate to use helper function
  if (data.type === 'statsUpdate' && data.players) {
    const clientSafePlayers = {};
    for (const id in data.players) {
      clientSafePlayers[id] = getPlayerStateForClient(data.players[id]);
    }
    const message = JSON.stringify({ ...data, players: clientSafePlayers });
    wss.clients.forEach(client => {
      if (client.readyState === WebSocket.OPEN) client.send(message);
    });
    return; // Exit after handling statsUpdate
  }

  // Use viewport updates for efficiency on player/monster changes
  if (data.type === 'players' || data.type === 'update' || data.type === 'stateUpdate') {
    return broadcastView('update'); // Consolidate into a single 'update' viewport broadcast
  }

  // For other message types, send to all clients
...
// Broadcast full game state for a specific area (only used for major changes like sprite changes)
function broadcastState(areaId) {
  const areaMonsters = areas[areaId]?.monsters || [];
  const areaGrid = areas[areaId]?.grid;
  if (!areaGrid) return; // Don't broadcast if area isn't loaded

  // Create a client-safe version of players in this area
  const areaPlayers = {};
  for (const id in players) {
    if (players[id].area === areaId) {
      areaPlayers[id] = getPlayerStateForClient(players[id]); // Use helper here
    }
  }

  const data = JSON.stringify({
    type: 'stateUpdate', // Client needs to handle this type
    players: areaPlayers,
    monsters: areaMonsters
    // Don't send grid here, rely on viewport updates
  });

  wss.clients.forEach(client => {
    if (client.readyState === WebSocket.OPEN && players[client.playerId]?.area === areaId) {
      // Send full state only to players in the relevant area
      client.send(data);
    }
  });

  // Note: We no longer broadcast stats separately here; broadcastState
  // is called alongside specific actions (like sprite change) which
  // already trigger a statsUpdate via the main broadcast function if needed.
}
...
function doMonsterMove(areaId, m) {
  ...
  if (m.type === 'Sorrow') {
    ...
    for (const pos of moves) {
      ...
      // AREA 00001 BOUNDARY CHECK (Block only if BOTH > 40)
      if (areaId === '00001' && (pos.x > 40 && pos.y > 40)) continue;
...
function handleQuit(ws) {
  ...
  const quitData = {
    name: p.name,
    // Save stats at the root level to match current loading logic
    faith: p.faith,
    health: p.health,
    buster: p.buster,
    wall: p.wall,
    filler: p.filler,
    position: { x: p.x, y: p.y }, // Keep position nested
    area: p.area, // Add the player's current area
    spriteCoords: p.spriteCoords, // Keep spriteCoords at the root
    ipLoginCounts: p.ipLoginCounts // Save the updated IP counts
  };
  const safeIp = p.ip.replace(/[^a-zA-Z0-9]/g, '_'); // TODO: Need to get IP correctly first
