...
// Level and map file naming for area maps
const LEVEL_ID = 1;
const AREA_ID = String(LEVEL_ID).padStart(5, '0');
const AREA_FILE = `Area${AREA_ID}.map`;
const AREA_BACKUP_PREFIX = `Area${AREA_ID}`;
...
// Load or create area map
const areaPath = path.join(__dirname, AREA_FILE);
let grid;
if (fs.existsSync(areaPath)) {
  const data = fs.readFileSync(areaPath, 'utf8');
  grid = data.split('\n').map(line => line.split(''));
  if (DEBUG >= 2) console.log('Loaded area map from', AREA_FILE);
} else {
  grid = generateGrid();
  placeDoors(grid);
  const ascii = grid.map(row => row.join('')).join('\n');
  fs.writeFileSync(areaPath, ascii);
  if (DEBUG >= 2) console.log('Saved new area map as', AREA_FILE);
}
...
// spawn initial monsters and enforce counts per type
function spawnMonsters() {
  const empties = [];
  for (let yy = 0; yy < gridSize; yy++) {
    for (let xx = 0; xx < gridSize; xx++) {
      if (grid[yy][xx] === '.') {
        const occupiedByPlayer = Object.values(players).some(p => p.x === xx && p.y === yy);
        const occupiedByMonster = monsters.some(m => m.x === xx && m.y === yy);
        if (!occupiedByPlayer && !occupiedByMonster) empties.push({ x: xx, y: yy });
      }
    }
  }
  for (const type in monsterTypes) {
    const cfg = monsterTypes[type];
    const desired = cfg.count || 0;
    const current = monsters.filter(m => m.type === type).length;
    const toSpawn = desired - current;
    for (let i = 0; i < toSpawn && empties.length > 0; i++) {
      const idx = Math.floor(Math.random() * empties.length);
      const { x, y } = empties.splice(idx, 1)[0];
      const id = `${type}_${Date.now()}_${i}`;
      monsters.push({ id, type, x, y, health: cfg.health, moveCounter: 0 });
      if (DEBUG >= 1) console.log(`[Monsters] Spawned ${type} at (${x},${y})`);
    }
  }
}
// initial spawn
...
function saveMap() {
  ...
  const backupName = `${AREA_BACKUP_PREFIX}-${hh}${mm}.map`;
