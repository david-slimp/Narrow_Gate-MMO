...
document.addEventListener('keydown', event => {
  ...
  if (['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight'].includes(key)) {
    ws.send(JSON.stringify({ type: 'move', direction: key.slice(5).toLowerCase() }));
...
function renderGrid(grid) {
  ...
  container.innerHTML = '';
  const cx = currentPlayer ? currentPlayer.x : Math.floor(gridSize/2);
  const cy = currentPlayer ? currentPlayer.y : Math.floor(gridSize/2);
  for (let dy = -viewRadius; dy <= viewRadius; dy++) {
    for (let dx = -viewRadius; dx <= viewRadius; dx++) {
      const worldY = cy + dy;
      const worldX = cx + dx;
      let val;
      if (worldY < 0 || worldY >= gridSize || worldX < 0 || worldX >= gridSize) val = '#';
      else val = grid[worldY][worldX];
